...
// Implementing the trace system call
// Traces the system calls that are being called by a process
// and have been flagged to be traced
int trace(int pid, int mask)
{
  ...
  {
    acquire(&p->lock);
    // printf("Acquire lock for pid %d\n", p->pid);
    if (p->pid == pid)
    {
      // printf("set trace flag to %d and trace mask to %d\n", 1, mask);
      p->traceMask = mask;
      p->toTrace = 1;
      breaker = 1;
    }
