...
extern uint64 sys_sigalarm(void);     // declared helper function
extern uint64 sys_sigreturn(void);    // declared helper function
...
void syscall(void)
{
  ...
  if (num > 0 && num < NELEM(syscalls) && syscalls[num])
  {
    ...
    // stores 1st argument of syscall in a0 register
    int prevArg = p->trapframe->a0;
    // now a0 stores return value
    p->trapframe->a0 = syscalls[num]();
    // if trace is set and if the numth bit is 1 in mask
    if (p->toTrace && ((1 << num) & p->traceMask) > 0)
    {
      // prints pid and name
      printf("%d: syscall %s (", p->pid, syscalls_name[num]);
      // checks for arguments and prints them
      if (syscalls_args[num] >= 1)
      {
        printf("%d", prevArg);
      }
      if (syscalls_args[num] >= 2)
      {
        printf(" %d", p->trapframe->a1);
      }
      if (syscalls_args[num] >= 3)
      {
